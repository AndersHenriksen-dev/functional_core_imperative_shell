...
from insert_package_name.core import register_reader
...
from insert_package_name.domains.example_domain.pipeline import run
...
Run with:
```bash
python -m insert_package_name.main active_domains=[s3_domain]
```
...
Run with:
```bash
python -m insert_package_name.main -cn config_prod
```
